...
It produces TWO things:
  1. prep.db   -- a ready-to-use SQLite database (zero-install fallback)
  2. seed.sql  -- portable INSERT statements to load into Postgres
                  (Postgres is the primary target: psql -d prep -f schema.sql -f seed.sql)

Deterministic: a fixed RNG seed means everyone gets identical data, so the
solutions files produce the same answers you will.
"""
import os
import random
import sqlite3
...
def main():
    ...
    # --- also emit a portable seed.sql for Postgres ---------------------------
    tables = {
        "regions": regions,
        ...
        "usage_metrics": usage_rows,
        "cost_records": cost_rows,
        "incidents": incidents,
    }
    write_seed_sql(SEED_PATH, tables)

    # summary
    print(f"Built {DB_PATH}")
    print(f"Wrote {SEED_PATH}  (for Postgres: psql -d prep -f schema.sql -f seed.sql)")
    for tbl in tables:
...
def _sql_literal(v):
    """Render a Python value as a SQL literal (portable across SQLite/Postgres)."""
    if v is None:
        return "NULL"
...
def write_seed_sql(path, tables):
    """Emit batched INSERT statements that load into schema.sql (Postgres) and
    also run fine on SQLite. Truncates first so it's re-runnable."""
    order = list(tables.keys())
    with open(path, "w") as f:
        f.write("-- Generated by build_db.py. Load AFTER schema.sql.\n")
        f.write("-- Deterministic data; do not edit by hand.\n\n")
        # clear in reverse FK order
        f.write("DELETE FROM " + ";\nDELETE FROM ".join(reversed(order)) + ";\n\n")
        for tbl in order:
            rows = tables[tbl]
            if not rows:
                continue
            f.write(f"INSERT INTO {tbl} VALUES\n")
            lines = ["    (" + ", ".join(_sql_literal(c) for c in r) + ")"
                     for r in rows]
            f.write(",\n".join(lines) + ";\n\n")
